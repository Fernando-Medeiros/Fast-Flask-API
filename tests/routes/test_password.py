...
from app.models.user import RequestRecoverPassword
...
class TestRecover:
    v_data = RequestRecoverPassword(**CaseLogin.valid_user).dict()
    i_data = RequestRecoverPassword(**CaseLogin.invalid_user()).dict()

    path = UrlPassword.recover

    # VALID
    def test_recover_send_email(self, client_one):
        response = client_one.post(self.path, json=self.v_data)
    ...
    def test_recover_invalid_data(self, client_one):
        response = client_one.post(self.path, json=self.i_data)
    ...
    def test_recover_without_user(self, client):
        response = client.post(self.path, json=self.v_data)
...
class TestUpdate:
    ...
    def test_update_password(self, client_two_auth):
        response = client_two_auth.patch(self.path, json=self.v_data)
    ...
    def test_update_invalid_password(self, client_two_auth):
        response = client_two_auth.patch(self.path, json=self.i_data)
    ...
    def test_update_without_auth(self, client_one):
        response = client_one.patch(self.path, json=self.v_data)
...
class TestReset:
    ...
    v_token: str = TokenJwt.create_recover_token(sub=username)
    i_token: str = TokenJwt.create_recover_token(sub=username, exp=expire)

    v_data = {"password": "Test@123", "confirm": "Test@123"}
